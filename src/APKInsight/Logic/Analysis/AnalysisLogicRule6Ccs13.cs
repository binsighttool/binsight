...
namespace APKInsight.Logic.Analysis
{
    ...
    /// </summary>
    public class AnalysisLogicRule6Ccs13 : AnalysisLogic
    {
        ...
        {
            var useCases = ApkInfo.UseCases.Where(uc => uc.IsCipherUseCaseRule6).ToList();
        ...
        private void ProcessUseCase(UseCase useCase)
        {
            ...
            if (ProcessFileForUseCase(useCase))
            {
                ...
                {
                    case "Ljava/security/SecureRandom;->setSeed([B)V":
                        seedRegIdx = 1;
                        break;
                    case "Ljava/security/SecureRandom;->setSeed(J)V":
                        seedRegIdx = 1;
                        break;
                    case "Ljava/security/SecureRandom;-><init>([B)V":
                        seedRegIdx = 1;
                        break;
                    default:
                        _report.AddLineWithoutCounter($"{ApkInfo.Id},{useCase.InMethodPos},{useCase.Filename},NotRelevant,");
                        return;
                }
                var saltSlices = SliceProgramBack(seedRegIdx);
